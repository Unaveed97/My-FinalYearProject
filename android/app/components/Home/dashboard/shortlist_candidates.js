...
import recommendation_Recruiter from './recommendation_recuiter';
import UserDetail from './userDetail';
import { Title } from 'react-native-paper'
import LinearGradient from 'react-native-linear-gradient';

export class shortlist_candidates extends Component {
    constructor(props) {
        super(props)
        this.state = {
            data: []
    ...
    render() {
        ...
        return (
            <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} colors={['#e0e0e0', '#3949ab', '#e0e0e0']} style={{ flex: 2.5 }}>
                <View>
                    <Title style={styles.loginSty} >Select a Job to view its Shortlisted Candidates</Title>
                    <FlatList
...
const Temp_stack = createStackNavigator(
    {
        Form1: { screen: shortlist_candidates },
        Form2: { screen: recommendation_Recruiter },
        Form3: { screen: UserDetail },
